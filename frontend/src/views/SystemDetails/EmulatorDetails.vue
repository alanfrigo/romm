...
import type { Emulator } from "@/stores/systems";
...
const { currentEmulator } = storeToRefs(systemsStore);
...
async function fetchDetails() {
  ...
  try {
    const { data } = await systemApi.getEmulator(id);
    currentEmulator.value = data;
...
async function saveSummary() {
  if (!currentEmulator.value) return;
  try {
    const { data } = await systemApi.updateEmulator({
      id: currentEmulator.value.id,
...
function cancelEditSummary() {
  summaryText.value = currentEmulator.value?.summary || "";
  editingSummary.value = false;
}

function getPlatformName(platformId: number): string {
  const platform = platformsStore.get(platformId);
  return platform?.display_name || platform?.name || `Platform #${platformId}`;
}
...
<template>
  ...
  <v-container v-else-if="notFound" class="text-center mt-16">
    ...
    <h2 class="text-h5 mt-4">Emulator not found</h2>
    <v-btn class="mt-4" color="primary" @click="router.back()">Go back</v-btn>
  </v-container>

  <!-- Detail page -->
  <template v-else-if="currentEmulator">
    <!-- Background header -->
    <div
      class="bg-toplayer"
    ...
    >
      <v-img
        v-if="currentEmulator.path_cover_large"
        :src="currentEmulator.path_cover_large"
        cover
      ...
      >
        ...
        >
          <v-card rounded="lg" class="overflow-hidden">
            ...
            >
              <v-icon size="80" color="grey">mdi-controller</v-icon>
            </div>
          </v-card>

          <!-- Associated platforms -->
          <v-card
            v-if="currentEmulator.platform_ids.length > 0"
            class="mt-4 pa-3"
            rounded="lg"
            variant="outlined"
          >
            <div class="text-subtitle-2 mb-2">Compatible Platforms</div>
            <v-chip
              v-for="pid in currentEmulator.platform_ids"
              :key="pid"
              class="mr-1 mb-1"
              size="small"
              label
      ...
      <v-col class="flex-grow-1">
        ...
        >
          ...
          >
            ...
            <v-tab
              v-if="currentEmulator.merged_screenshots.length > 0"
              value="screenshots"
            >
              Screenshots
            </v-tab>
          ...
          <v-col cols="12" class="px-1">
            <v-window v-model="tab" disabled class="py-2">
                ...
                <v-row no-gutters>
                  <v-col>
                      ...
                      <v-table density="compact">
                        <tbody>
                          <tr>
                            ...
                            <td>{{ currentEmulator.file_name }}</td>
                          </tr>
                          <tr>
                            <td class="text-caption font-weight-bold">Path</td>
                            <td>{{ currentEmulator.file_path }}</td>
                          </tr>
                          <tr>
                            <td class="text-caption font-weight-bold">Size</td>
                            <td>
                              {{
                                formatBytes(currentEmulator.file_size_bytes)
                              }}
                            </td>
                          </tr>
                          <tr>
                            ...
                            <td>
                              {{ currentEmulator.file_extension || "—" }}
                            </td>
                          </tr>
                        </tbody>
                      </v-table>
                    ...
                    <v-card class="mb-4 pa-4" rounded="lg" variant="outlined">
                      ...
                      <p
                        v-else-if="currentEmulator.summary"
                        class="text-body-2"
                      >
                        {{ currentEmulator.summary }}
                      </p>
              ...
              <v-window-item value="screenshots">
                <v-row>
                  <v-col
                    v-for="(screenshot, idx) in currentEmulator.merged_screenshots"
                    :key="idx"
                    cols="12"
                    sm="6"
                    md="4"
